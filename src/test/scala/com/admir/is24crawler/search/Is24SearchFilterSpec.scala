...
import com.admir.is24crawler.models.ResultPagePath

// TODO: whole test could be done better
class Is24SearchFilterSpec extends FlatSpec with Matchers with SprayJsonSupport with DefaultJsonProtocol {
  ...
  it should "be possible to create a search filter from json" in {
    val filter = Is24SearchFilter(firstPageReqBodyJson)
    ...
    val rpl = ResultPagePath(Await.result(firstPageLocationFut, 5.seconds))
